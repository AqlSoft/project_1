...
@endsection
@section('links')
    <button class="btn btn-sm btn-primary"><a href="{{ route('receipts.input_receipts', [1]) }}"><span class="btn-title">العودة
                إلى
            ...
            <fieldset class="">
                ...
                <div id="tableQuery" class="text-right">
                    ...
                    <div class="result" id="tableBusy" style="display: none">
                        الطبلية رقم: &nbsp;
                        <b class="tableNumber">000</b>&nbsp; موجودة، و <b id="contract"></b> عدد <b id="load"></b>
                        كرتون
                    </div>
                </div>
        ...
        <fieldset class="">
            ...
            <div class="row receipt_info">
                ...
                <div class="col col-4">
                    ...
                    <span class="data">{{ mb_substr($receipt->theClient->name, 0, 25) }}</span>
                </div>
            ...
            @if (count($entries))
                ...
                @endforeach
                <div class="buttons">
                    <button class="btn btn-sm font-weight-bold btn-success">
                        <a href="{{ route('receipts.input_receipts', $receipt->id) }}"> العودة لسندات الاستلام </a>
                    </button>
                    @if ($receipt->confirmation == 'approved')
                        <button class="btn btn-sm font-weight-bold btn-primary"><a
                                href="{{ route('receipts.input.print', $receipt->id) }}"> طباعة السند </a></button>
                    @endif
                    @if ($receipt->confirmation !== 'approved')
                        <button class="btn btn-sm font-weight-bold btn-primary"><a
                                href="{{ route('receipts.input.print', $receipt->id) }}">اعتماد وطباعة</a></button>
                    @endif
                </div>
            @else
...
@section('script')
    <script type="text/javascript">
        $(document).on('keyup', '#table', function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response == '') {
                            ...
                            $('#table_serial').val($('#table').val().toString().padStart(10, '450000'))
            ...
            let number = 2
            let result = number.toString().padStart(10, '450000')



        });
        $(document).on('click', '#submitEntry', function() {
            window.localStorage.tableName = ''
        })
        $(document).on('click', '#save_table', function() {
            window.localStorage.tableName = $('#tableName').val()

        })
        $(document).ready(function() {
            
            $('#table').focus();
            $('#table').val(window.localStorage.tableName)
        })
    </script>
@endsection
