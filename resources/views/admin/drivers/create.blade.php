        ...
        <div class="quick-nav input-group fs-6 py-2 w-100">
            ...
            <span class="text-center btn form-control btn-outline-primary">
                <a class="d-inline-block py-1" href="{{ route('drivers.home') }}">
                    <i class="fas fa-folder-open"></i> &nbsp; انشاء سند جديد &nbsp;
                </a>
            </span>

            <span class="text-center btn form-control btn-primary">
                <a class="d-inline-block py-1" href="{{ route('reception.home', [1]) }}">
                    <i class="fas fa-folder-plus"></i> &nbsp; سندات الادخال &nbsp;
                </a>
            </span>
        </div>
        <fieldset style="width: 90%">
            ...
            <form class="" action="{{ route('drivers.store') }}" method="POST">
                ...
                <div id="" class="mt-3 input-group">
                    ...
                    <input class="form-control" type="search" name="e_name" id="driver-e-name" placeholder="John Doe!">
                </div>
                ...
                <div id="receipt-date" class="mt-3 input-group">
                    ...
                    <select class="form-control" name="nationality" id="nationality">
                        @foreach ($trucks as $type)
                            <option value="{{ $type->id }}"> {{ $type->a_name }} - {{ $type->e_name }}</option>
                        @endforeach
                    </select>


                    <label for="car-panel" class="input-group-text"> رقم اللوحة</label>
                    <input class="form-control" type="search" name="car_panel" id="car-panel" placeholder="A B C 1250">

                    <label for="car-model" class="input-group-text"> الموديل</label>
                    <input class="form-control" type="search" name="car_model" id="car-model" placeholder="Mercedes">
                </div>
                ...
                <div id="" class="mt-3 input-group">
                    ...
                    <input class="form-control" type="search" name="iqama" id="iqama" placeholder="2123123123">

                    <label for="nationality" class="input-group-text">الجنسية</label>
                    <select class="form-control" name="nationality" id="nationality">
                        @foreach ($contries as $country)
                            <option value="{{ $country->id }}">{{ $country->iso }} - {{ $country->a_name }}</option>
                        @endforeach
                    </select>
