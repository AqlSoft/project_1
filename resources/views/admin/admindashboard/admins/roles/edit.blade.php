                ...
                <form action="{{ route('roles.update') }}" method="post">
                    ...
                    <div class="form-section row bg-light p-3 mx-3 mt-4" style="box-shadow: 0 0 6px 2px #ccc inset">
                        ...
                        <div class="form-floating mt-3">
                            <textarea style="height: 150px" class="form-control" name="brief" id="brief" placeholder="">{{ $role->brief }}</textarea>
                            <label for="brief">نبذة عن الدور:</label>
                        </div>

                        <div class="form-check form-switch form-check-reverse mt-3" dir="rtl">
                            <input class="form-check-input" type="checkbox" role="switch" name="status" id="role_status"
                                {{ $role->status ? 'checked' : '' }}>
                            <label class="form-check-label" for="role_status"> الحالة: <span
                                    id="statusText">{{ $role->status ? 'مفعلة' : 'معطلة' }}</span></label>
                        </div>
                    </div>
                    ...
                    <div class="buttons pb-0" style="justify-content: flex-end">
                        ...
                        <button id="dismiss_btn" class="btn btn-outline-info"
                            onclick="window.location='{{ route('roles.home') }}'" type="button"
