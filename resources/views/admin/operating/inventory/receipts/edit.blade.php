...
@section('links')
    ...
    <button class="btn btn-sm btn-primary"><a href="{{ route('receipts.output_receipts', [1]) }}">
            <i data-bs-toggle="tooltip" data-bs-title="العودة إلى سندات الإخراج" class="fas fa-file-invoice-dollar"></i>
        </a></button>
@endsection
        ...
        <fieldset style="width: 90%">
            ...
            <form action="{{ route('receipts.updateInputReceipt') }}" method="POST">
                ...
                <div class="input-group mt-3">
                    <label class="input-group-text required" for="hij_date_input"> اسم السائق: </label>
                    <input class="form-control" type="text" name="driver" placeholder="اسم السائق" required
                        value="{{ $item->driver == null ? 'غير معروف' : $item->driver }}">
                </div>
                <div class="input-group mt-3">
                    <label class="input-group-text required" for="hij_date_input"> سبب الأخراج أو الوجهة: </label>
                    <input class="form-control" type="text" name="farm" placeholder="اسم المزرعة / المصدر" required
                        value="{{ $item->farm == null ? 'غير معروف' : $item->farm }}">
                </div>
                <div class="input-group mt-3">
                ...
                <div class="input-group mt-3">
                    ...
                    <input type="text" class="form-control text-primary fw-bold" name="s_number"
                        value="{{ $item->s_number }}">
