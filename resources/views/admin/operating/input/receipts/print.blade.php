		...
		<div class="receipt-container">
			...
			<div class="receipt position-relative" dir="rtl">
				...
				<div class="controls" dir="ltr">
					...
					<div class="buttons">
						<button class="btn btn-outline-primary" onclick="window.location='{{ route('reception.entries.create', [$receipt->id, 0]) }}'">
							العودة للسند
						</button>
						@if ($receipt->confirmation !== 'approved')
							<button class="btn btn-outline-primary" onclick="window.location='{{ route('reception.approve', [$receipt->id]) }}'"> اعتماد
							</button>
						@endif
						@if ($receipt->confirmation == 'approved')
							<button class="btn  mx-1 btn-outline-primary" onclick="window.print()"> طباعة</button>
						@endif
						<button class="btn btn-outline-primary" onclick="window.location='{{ route('reception.home', [1]) }}'">الذهاب
							للسندات</button>
						<button class="btn btn-outline-primary" onclick="window.location='{{ route('contract.view', [$receipt->contract_id, 2]) }}'">الذهاب
							للعقد</button>
