			...
			<fieldset class="mt-5 mx-3">
				...
				@if (count($entries))
					@foreach ($entries as $index => $entry)
						<form class="mt-1" id="form_{{ $index }}" action="{{ route('stores.save.table.position') }}" method="POST">
							@csrf
							...
							<div class="input-group">
								...
								<label class="input-group-text" style="width: 3rem" value="">{{ $entry->inputs }}</label>
								<label class="input-group-text"> الغرفة </label>
								<select class="form-control" id="" name="room">
									@foreach ($rooms as $room)
										<option {{ $entry->storingRecord ? ($entry->storingRecord->room == $room->id ? 'selected' : '') : null }} value="{{ $room->id }}">
											{{ $room->code }}</option>
									@endforeach
								</select>
								<label class="input-group-text"> المكان </label>
								<select class="form-control" id="" name="partition">
									<option {{ $entry->storingRecord ? ($entry->storingRecord->partition == 'L' ? 'selected' : '') : null }} value="L">Left</option>
									<option {{ $entry->storingRecord ? ($entry->storingRecord->partition == 'R' ? 'selected' : '') : null }} value="R">Right</option>
									<option {{ $entry->storingRecord ? ($entry->storingRecord->partition == 'F' ? 'selected' : '') : null }} value="F">Front</option>
								</select>
								...
								<select class="form-control" id="" name="rack">
									<option {{ $entry->storingRecord ? ($entry->storingRecord->rack == 'A' ? 'selected' : '') : null }} value="A">A</option>
									<option {{ $entry->storingRecord ? ($entry->storingRecord->rack == 'B' ? 'selected' : '') : null }} value="B">B</option>
									<option {{ $entry->storingRecord ? ($entry->storingRecord->rack == 'C' ? 'selected' : '') : null }} value="C">C</option>
									<option {{ $entry->storingRecord ? ($entry->storingRecord->rack == 'D' ? 'selected' : '') : null }} value="D">D</option>
								</select>
