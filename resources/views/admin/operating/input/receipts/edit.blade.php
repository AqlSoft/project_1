...
@section('content')
    <div class="container pt-5">
        ...
        <div class="tab-content" id="nav-tabContent" style="background:#fff">
            ...
            <form action="{{ route('reception.update') }}" method="POST">
                <input type="hidden" name="id" value="{{ $receipt->id }}">
                @csrf
                <div id="receipt_date" class="mt-3 input-group">
                    <label class="input-group-text" for="greg_date_picker">بتاريخ:</label>
                    <label class="input-group-text p-0 m-0">
                        <input type="date" name="greg_date" style="" id="greg_date_picker" value="">
                    </label>
                    <label class="input-group-text">
                        <span id="greg_date_display">{{ $receipt->greg_date }}</span>
                        <input type="hidden" name="greg_date" id="greg_date_input" value="{{ $receipt->greg_date }}">
                    </label>
                    <label class="input-group-text">الموافق:</label>
                    <label class="input-group-text">
                        <span id="hijri_date_display">{{ $receipt->hij_date }}</span>
                        <input type="hidden" name="hijri_date" value="{{ $receipt->hij_date }}" id="hijri_date_input">
                    </label>
                    <label for="" class="input-group-text">سند</label>

                    <select name="type" id="type" class="form-control">
                        <option value="1">استقبال البضاعة</option>
                    </select>
                ...
                <div class="input-group mt-1">
                    ...
                    <button type="submit" value="حفظ" class="input-group-text"> تحديث البيانات </button>
                </div>
            </form>
            <div class="buttons mt-3">
                <button class="btn btn-sm btn-outline-primary">
                    <a href="{{ route('reception.home', [1]) }}">الذهاب للسندات</a>
                </button>
                <button class="btn btn-sm btn-outline-primary">
                    <a href="{{ route('contract.view', [$receipt->contract_id, 2]) }}">الذهاب للعقد</a>
                </button>
            </div>
