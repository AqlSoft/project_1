            ...
            <form action="{{ route('stocking.store') }}" method="POST">
                ...
                <div id="receipt_date" class="mt-3 input-group">
                    ...
                    <select name="type" id="type" class="form-control">
                        <option value="6">جرد بضاعة عميل</option>
                    </select>
                </div>
                {{-- {{ $contracts }} --}}
                ...
                <div class="input-group mt-1">
                    ...
                    <select class="form-control" id="contact" name="contact" placeholder="المندوب">
                        ...
                        @foreach ($contacts as $item)
                            <option {{ old('contact') == $item->id ? 'selected' : '' }} value="{{ $item->id }}">
                                {{ $item->name . ' - ' . $item->rule }}</option>
                        @endforeach
                    </select>

                    <label class="input-group-text" for="reason">سبب الجرد:</label>
                ...
                <div class="input-group mt-1">
                    ...
                    <label class="input-group-text" id="s_number_label" data-serial="{{ $lir }}"> الرقم
                        المسلسل:
                        <input type="number" name="s_number" id="s_number_input"
                            value="{{ old('s_number') == $lir ? $lir : old('s_number') }}">
