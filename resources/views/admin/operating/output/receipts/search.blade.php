				...
				<tr>
					...
					<td>
						<a class="btn btn-sm displayReceipt btn-block p-1 mb-1 bg-secondary" class="fa fa-eye text-primary" data-receipt-id="{{ $item->id }}"
							data-search-token="{{ csrf_token() }}" data-search-url="{{ route('reception.info') }}" data-tab="1" data-bs-toggle="tooltip"
					...
					<td><a class="btn btn-sm btn-block p-1 mb-1 bg-secondary" data-bs-toggle="tooltip" data-bs-title="رؤية العقد"
							href="{{ route('contract.view', [$item->contract_id, 1]) }}">العقد:
							{{ $item->the_contract }}</a>
					...
					<td>{{ $item->total_outputs ? $item->total_outputs : 0 }}</td>
					<td>
						@if ($tab == 1)
							...
							<a class="displayReceipt" data-receipt-id="{{ $item->id }}" data-search-token="{{ csrf_token() }}"
								data-search-url="{{ route('delivery.info') }}" data-tab="1"><i class="fa fa-eye text-primary" data-bs-toggle="tooltip"
